...
The Operator uses four prompt templates (assess, roadmap, generate-prompts, judge).
By default these are built-in, but you can override any or all of them by placing
Markdown files in a directory and passing `--operator-prompts-dir`:

```bash
uv run python -m release_flow --assess --operator-prompts-dir operator_prompts
```

The directory should contain one or more of these files:

| File | Overrides | Placeholders available |
|---|---|---|
| `assess.md` | Assessment prompt | `{local_path}` |
| `roadmap.md` | Roadmap prompt | `{local_path}`, `{assessment}` |
| `generate_prompts.md` | Prompt-generation prompt | `{roadmap}` |
| `judge.md` | Judging prompt | `{agent_prompt}`, `{changes_summary}`, `{files_changed}` |

Any file that is missing is silently skipped — the built-in default is used instead.
A starter set of templates is provided in the `operator_prompts/` directory.

### Operator constitution

The Operator supports a **constitution file** — a set of first principles that are
prepended to every prompt the Operator sends. This ensures the Operator always
operates within defined guardrails, regardless of which task it is performing.

```bash
uv run python -m release_flow --assess --constitution operator_prompts/constitution.md
```

The constitution is injected before every prompt (assess, roadmap, generate, judge),
so the LLM always has the first principles in context. A default constitution is
provided in `operator_prompts/constitution.md` covering:

- Do no harm — never weaken security or remove safety checks
- Preserve correctness — favour working solutions over clever ones
- Maintain transparency — always explain reasoning
- Respect scope — stay within the prompt's boundaries
- Security first — treat every change through a security lens
- Test coverage is non-negotiable
- Human-in-the-loop — bias toward keeping reviewers in control
- Incremental progress — many small changes over few large ones
- Model independence — prompts must work with any LLM
...
--operator-prompts-dir DIR Directory with custom prompt templates (assess.md, etc.)
--constitution FILE        First-principles file prepended to every Operator prompt
...
Operator Options (all optional):
  ...
  --operator-prompts-dir  Directory with custom Operator prompt templates
...
operator_prompts/    # Customisable Operator prompt templates
├── assess.md        #   Codebase assessment prompt
├── roadmap.md       #   Roadmap definition prompt
├── generate_prompts.md  # Agent prompt generation prompt
├── judge.md         #   Change evaluation / judging prompt
├── constitution.md  #   First principles the Operator must follow
