...
class ReleaseFlow:
    ...
    def ensure_clean_state(self) -> None:
        ...
        Preserves the prompts file across git operations so that
        operator-generated prompts are not lost by stash/reset.
        ...
        # Preserve the prompts file — it may have been generated by the
        # operator and is not yet committed, so stash/reset would discard it.
        prompts_path = self.local_path / self.config.operator.prompts_file
        prompts_backup = None
        if prompts_path.is_file():
            prompts_backup = prompts_path.read_text(encoding="utf-8")
        ...
        try:
            ...
            # Restore the prompts file if we backed it up
            if prompts_backup is not None:
                prompts_path.write_text(prompts_backup, encoding="utf-8")
                logger.info(f"Restored {prompts_path.name} after git reset")
